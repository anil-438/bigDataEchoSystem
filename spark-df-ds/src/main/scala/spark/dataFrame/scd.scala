...
object scd {
  ...
  val history = spark.read.option("delimiter","|").schema(empSchema).csv("C:\\Users\\me\\IdeaProjects\\bigData\\src\\main\\resources\\history_data.txt")
  val delta=spark.read.option("delimiter","|").schema(empSchema).csv("C:\\Users\\me\\IdeaProjects\\bigData\\src\\main\\resources\\deltaFile.txt").drop("surrogate_key")
  ...
  def scd1: Unit ={
    sc.setLogLevel("off")
      ...
      val nonModified =spark.sql(s"""select history.* from history anti join delta_01 on history.empno = delta_01.empno""")

      nonModified.union(newRecords).union(updatedRecords).distinct().sort("surrogate_key").show(false)
    }
  def scd2: Unit ={
